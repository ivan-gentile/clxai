...
# Seed-parameterized Triplet + Patch training for CIFAR-100
# IMPORTANT: Using semi-hard mining to prevent embedding collapse
SEED=${SEED:-0}
OUTPUT_DIR=${OUTPUT_DIR:-"results/models/cifar100_r152/triplet_patch_seed${SEED}"}
RUN_NAME=${RUN_NAME:-"triplet_cifar100_patch_seed${SEED}"}

echo "=========================================="
echo "CLXAI: Triplet + Patch Augmentation - CIFAR-100 + ResNet-152 (Seed ${SEED})"
echo "=========================================="
...
python src/training/train_scl.py \
    --config configs/triplet_cifar100_patch_aug.yaml \
    --epochs 500 \
    --batch_size 256 \
    --lr 0.1 \
