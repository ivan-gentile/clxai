...
# Triplet Loss + Gaussian Noise Augmentation Training
# Get seed from environment variable (default to 0)
SEED=${SEED:-0}
OUTPUT_DIR=${OUTPUT_DIR:-"results/models/triplet_noise_seed${SEED}"}
RUN_NAME=${RUN_NAME:-"triplet_noise_seed${SEED}"}

echo "=========================================="
echo "CLXAI: Triplet + Noise Augmentation (Seed ${SEED})"
echo "=========================================="
...
# Run training with seed and noise augmentation
echo "Starting Triplet + Noise training with seed ${SEED}..."
python src/training/train_scl.py \
    --config configs/scl_triplet_noise_aug.yaml \
    --epochs 500 \
    --batch_size 256 \
    --lr 0.1 \
    --loss triplet \
