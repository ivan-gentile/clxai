...
# Supervised Contrastive Learning + Gaussian Noise Augmentation Training
# Get seed from environment variable (default to 0)
SEED=${SEED:-0}
OUTPUT_DIR=${OUTPUT_DIR:-"results/models/scl_noise_seed${SEED}"}
RUN_NAME=${RUN_NAME:-"scl_noise_seed${SEED}"}

echo "=========================================="
echo "CLXAI: SupCon + Noise Augmentation (Seed ${SEED})"
echo "=========================================="
...
python src/training/train_scl.py \
    --config configs/scl_supcon_noise_aug.yaml \
    --epochs 500 \
    --batch_size 256 \
    --lr 0.5 \
    --temperature 0.07 \
