...
# Seed-parameterized CE + F-Fidelity Pixel Removal training
# 10% scattered pixel removal (exact F-Fidelity implementation)
SEED=${SEED:-0}
OUTPUT_DIR=${OUTPUT_DIR:-"results/models/ce_pixel_seed${SEED}"}
RUN_NAME=${RUN_NAME:-"ce_pixel_seed${SEED}"}

echo "=========================================="
echo "CLXAI: CE + F-Fidelity Pixel Augmentation (Seed ${SEED})"
echo "=========================================="
