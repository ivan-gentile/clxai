...
# Seed-parameterized CE + Patch training for CIFAR-100
SEED=${SEED:-0}
OUTPUT_DIR=${OUTPUT_DIR:-"results/models/cifar100_r152/ce_patch_seed${SEED}"}
RUN_NAME=${RUN_NAME:-"ce_cifar100_patch_seed${SEED}"}

echo "=========================================="
echo "CLXAI: CE + Patch Augmentation - CIFAR-100 + ResNet-152 (Seed ${SEED})"
echo "=========================================="
...
python src/training/train_ce.py \
    --config configs/ce_cifar100_patch_aug.yaml \
    --epochs 200 \
    --batch_size 128 \
    --lr 0.1 \
