...
# Seed-parameterized SupCon + Patch training for CIFAR-100
SEED=${SEED:-0}
OUTPUT_DIR=${OUTPUT_DIR:-"results/models/cifar100_r152/supcon_patch_seed${SEED}"}
RUN_NAME=${RUN_NAME:-"supcon_cifar100_patch_seed${SEED}"}

echo "=========================================="
echo "CLXAI: SupCon + Patch Augmentation - CIFAR-100 + ResNet-152 (Seed ${SEED})"
echo "=========================================="
...
python src/training/train_scl.py \
    --config configs/supcon_cifar100_patch_aug.yaml \
    --epochs 500 \
    --batch_size 256 \
    --lr 0.5 \
