...
# Seed-parameterized Triplet + Noise training for CIFAR-100
# IMPORTANT: Using semi-hard mining to prevent embedding collapse
SEED=${SEED:-0}
OUTPUT_DIR=${OUTPUT_DIR:-"results/models/cifar100_r152/triplet_noise_seed${SEED}"}
RUN_NAME=${RUN_NAME:-"triplet_cifar100_noise_seed${SEED}"}

echo "=========================================="
echo "CLXAI: Triplet + Noise Augmentation - CIFAR-100 + ResNet-152 (Seed ${SEED})"
echo "=========================================="
...
# Run training
echo "Starting Triplet + Noise training with seed ${SEED}..."
python src/training/train_scl.py \
    --config configs/triplet_cifar100_noise_aug.yaml \
    --epochs 500 \
    --batch_size 256 \
    --lr 0.1 \
