...
# Cross-Entropy + Gaussian Noise Augmentation Training
# Get seed from environment variable (default to 0)
SEED=${SEED:-0}
OUTPUT_DIR=${OUTPUT_DIR:-"results/models/ce_noise_seed${SEED}"}
RUN_NAME=${RUN_NAME:-"ce_noise_seed${SEED}"}

echo "=========================================="
echo "CLXAI: CE + Noise Augmentation (Seed ${SEED})"
echo "=========================================="
...
python src/training/train_ce.py \
    --config configs/ce_noise_aug.yaml \
    --epochs 200 \
    --batch_size 128 \
    --lr 0.1 \
