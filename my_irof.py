...
class IROF(Metric[List[float]]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.centroids = None  # New argument to support distance-based computations. To be set via set_centroids() method
    ...
    def set_centroids(self, centroids):
        """
        Set the class centroids for distance-based evaluation.
        
        Parameters
        ----------
        centroids: dict
            Dictionary mapping class indices to centroid tensors.
        """
    ...
    ########### new method to support distance-based prediction ###########
    def distance_to_centroid(self,
            all_centroids: np.ndarray,
            y_pred: np.ndarray,
            y_emb: np.ndarray) -> np.ndarray:
        """
        Calculate the Euclidean distance between the input's embedding and the centroid of its true class.

        Parameters
        ----------
        all_centroids : dict
            Dictionary mapping class labels to class centroid embeddings.
        y_pred : np.ndarray or int
            The true class label.
        y_emb : np.ndarray
            The input's embedding.
        ...
        if isinstance(y_emb, torch.Tensor):
            y_pred_embedding = y_emb.detach().cpu().numpy() 
        else:
            # Handle numpy array - flatten to 2D if needed
            if y_emb.ndim > 2:
                y_pred_embedding = y_emb.reshape(y_emb.shape[0], -1)
            elif y_emb.ndim == 1:
                y_pred_embedding = y_emb.reshape(1, -1)
            else:
                y_pred_embedding = y_emb  
        ...
        pred_centroid = all_centroids[y_pred]  # [D]
        distance = np.linalg.norm(y_pred_embedding - np.array(pred_centroid), axis=1)
        return distance
    
    ########### new method to support distance-based prediction ###########
    def get_prediction(self, y_emb):
        """
        Get the predicted class based on the closest centroid.
        Parameters
        ----------
        y_emb : np.ndarray
            The input's embedding.  
        ...
        """
        # Convert centroids dict to a stacked numpy array, sorted by class label
        centroid_array = np.array([
            self.centroids[k].numpy() if isinstance(self.centroids[k], torch.Tensor) else self.centroids[k]
            for k in sorted(self.centroids.keys())
        ])
        
        # minimal euclidean distance among all centroids
        distances = np.linalg.norm(centroid_array - y_emb, axis=1)
    ...
    def evaluate_instance(
        ...
        if self.distance_based:
            x_input = model.shape_input(x, x.shape, channel_first=True)
            y_emb = model.predict(x_input)  # Get embeddings/logits
            y_pred = self.get_prediction(y_emb)
            # For distance-based metric, use the true label to find the centroid
            if self.centroids is None:
                raise ValueError("Centroids not set. Call metric.set_centroids(centroids) before evaluation.")
            distance = self.distance_to_centroid(all_centroids = self.centroids, y_pred = y_pred, y_emb = y_emb)
            y_pred = distance
        ...
        for i_ix, s_ix in enumerate(s_indices):
            ...
            ### changes #####
            if self.distance_based:
                y_emb_perturb =  model.predict(x_input)
                y_pred_perturb = self.get_prediction(y_emb_perturb)
                distance_perturb = self.distance_to_centroid(all_centroids = self.centroids, y_pred = y_pred_perturb, y_emb = y_emb_perturb)
